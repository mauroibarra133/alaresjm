...
function Booking({ booking, fechaHoy: todayDate }) {
  // fechaHoy: YYYY-MM-DD
  //Constants
    ...
    if (dateBooking >= todayDate) {
      ...
      if (leftHour >= tiempoParaCancelar) {
        ...
        if (response.status == 200) {
          ...
          openModal(
            false,
            "Hubo un error al eliminar su reserva, intente mas tarde"
          );
        }
      } else {
        openModal(
          false,
          `No se puede cancelar la reserva faltando menos de ${tiempoParaCancelar} hora/s`
        );
      }
    }
  }
  async function handleUpdate() {
    if (dateBooking == todayDate) {
      ...
      if (leftHour >= tiempoParaCancelar) {
        ...
        openModal(
          false,
          `No se puede modificar la reserva faltando menos de ${tiempoParaCancelar} hora/s`
        );
      }
    } else {
  ...
  const formattedDate = `${dateBooking.substring(
    8,
    10
  )}/${dateBooking.substring(5, 7)}`;
    ...
    <div className="datos__body-row" key={booking.id}>
      ...
      <div className="datos__body-dato acciones">
        <img
          ...
          alt="Modificar"
          className={
            !validatePastHour(booking.hora, dateBooking) ? "old" : "new"
          }
      ...
      {showModal.isSubmitted && (
        <Modal
          isSubmitted={showModal.isClicked}
      ...
      {showModalUpdate && (
        <Overlay comp={"reserva"}>
          <ModalModificar
            ...
            reserva={booking}
            handleCloseModalUpdate={closeUpdateModal}
          ></ModalModificar>
        </Overlay>
      )}
    </div>
