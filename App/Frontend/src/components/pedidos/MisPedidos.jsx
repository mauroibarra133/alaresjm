...
function MisPedidos() {
  ...
  useEffect(() => {
    ...
    for (
      ...
      i <=
      Math.ceil(
        (filterOrders(pedidos).length == 0 ? 1 : filterOrders(pedidos).length) /
          LIMIT
      );
      i++
    ) {
  ...
  function handlePedidos() {
    if (pedidos.length <= 0) {
      return (
        <MisPedidosVacio
          goTo={"delivery"}
          msg={"Aun no tienes ningun pedido hoy"}
          msgButton={"PEDIR"}
        ></MisPedidosVacio>
      );
    } else {
      ...
      if (filteredOrders.length <= 0) {
        ...
        return filteredOrders.slice(offsett, LIMIT + offsett).map((order) => {
          return <Pedido pedido={order} key={order.id} />;
        });
      }
    }
  }
    ...
    <div className="mispedidos__container">
      ...
      <div className="mispedidos__form">
        <div className="mispedidos__filter">
          ...
          <p onClick={() => setFilterActive(!isFilterActive)}>
            {!isFilterActive
              ? "Ocultar pedidos antiguas"
              : "Mostrar pedidos antiguas"}
          </p>
        </div>
      ...
      {filterOrders().length > 0 && <div className="button">PEDIR</div>}
      {pages.length > 1 && (
