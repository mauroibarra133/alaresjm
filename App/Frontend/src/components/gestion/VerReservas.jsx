...
function VerReservas() {
  ...
  // Crear una nueva fecha que representa el día de mañana
  const tomorrow = new Date(today.getTime() + 24 * 60 * 60 * 1000);

  // Obtener la fecha del día de mañana en formato ISO (yyyy-mm-dd)
  ...
  return (
    <div className="verreservas veritems">
      <div className="verreservas__fechas veritems__fechas">
        <div
          className="verreservas__fecha-ayer button veritems__fechas-button"
          onClick={() => setFilterDate(todayDate)}
        >
          Hoy
        </div>
        <input
          className="verreservas__fecha-input"
          type="date"
          onChange={handleDate}
          value={filterDate}
        ></input>
        <div
          className="verreservas__fecha-hoy button veritems__fechas-button"
          onClick={() => setFilterDate(tomorrowDate)}
        >
          Mañana
        </div>
      </div>
      ...
      <div className="verreservas__reservas veritems__lista">
        <div
          className="verreservas__header veritems__header"
          style={{ gridTemplateColumns: "repeat(5, 1fr)" }}
        >
          <div className="veritems__header-column  verreservas__header-column">
            Hora
          </div>
          <div className="veritems__header-column  verreservas__header-column">
            Cant.
        ...
        <div className="verreservas__body veritems__body">
          {loading ? (
            ...
            <VerReservasVacio
              msg={"No hay reservas el día de hoy"}
              msgButton={":("}
            />
          ) : (
            filterBookings(bookings).map((booking) => (
              <div
                key={booking.id}
                className="veritems__row verreservas__row"
              ...
              >
                ...
                <div className="veritems__dato verreservas__dato">
                  <img
                    ...
                    alt={booking.estado}
                    className="status-img reserva-status-img"
                  />
                  {isLargeScreen && <p>{booking.estado}</p>}
                </div>
              </div>
            ))
          )}
