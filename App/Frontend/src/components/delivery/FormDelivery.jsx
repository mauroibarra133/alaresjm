  ...
  return (
    <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="order__form">
        <div className="form__row">
          ...
          <select
            ...
            {...register("tipoEntrega", {
              required: "Debes incluir como sera entregado",
            })}
          >
        ...
        <div className="form__row">
          ...
          <div>
            <input
              type="text"
              id={clientDirectionId}
              {...register("direccionCliente", {
                required: "Debes incluir la direccion",
                disabled: watch("tipoEntrega") !== "1",
                pattern: ADDRESS_REGEX,
              })}
            />
            {errors.direccionCliente?.type === "pattern" && (
              <p role="alert" className="form-error input-error">
                Algunos caracteres no estan permitidos
              </p>
            )}
            {errors.direccionCliente?.type === "required" && (
              <p role="alert" className="form-error input-error">
        ...
        <div className="form__row">
          ...
          <select
            ...
            id={typePayId}
            {...register("tipoPago", {
              required: "Debes incluir como pagar tu pedido",
            })}
          >
              ...
              {...register("montoEft", {
                ...
                validate: (value) => {
                  ...
                  if (parsedValue >= total * 2) {
                    ...
                    return "El monto a pagar debe ser superior al total";
                },
              })}
            />
      ...
      <div className="order__button">
        <button
          type="submit"
          className="order__confirmar button"
          disabled={total == 0}
        >
          {loading ? (
            <LoaderComponent size={"small"} color={"mp-color"} />
          ) : (
            "Confirmar pedido"
          )}
        </button>

        {preferenceId && watch("tipoPago") === "2" && (
          <Wallet
            initialization={{ preferenceId, redirectMode: "modal" }}
            className="button mp_button"
          />
        )}
      </div>
    </form>
